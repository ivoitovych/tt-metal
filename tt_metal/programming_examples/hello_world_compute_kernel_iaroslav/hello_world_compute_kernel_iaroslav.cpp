...
int main() {
    ...
    KernelHandle void_compute_kernel_id = CreateKernel(
        program,
        OVERRIDE_KERNEL_PREFIX "hello_world_compute_kernel_iaroslav/kernels/compute/void_compute_kernel.cpp",
        core,
        ComputeConfig{
    ...
    printf(
        "IAROSLAV, Hello, Core {0, 0} on Device 0, I am sending you a compute kernel. Standby awaiting "
        "communication.\n");

    // Wait Until Program Finishes, Print "Hello World!", and Close Device

    Finish(cq);
    printf("IAROSLAV, Thank you, Core {0, 0} on Device 0, for the completed task.\n");
    CloseDevice(device);
