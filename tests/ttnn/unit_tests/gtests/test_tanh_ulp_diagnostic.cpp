...
TEST_F(TanhUlpDeviceTest, ExhaustiveBf16Sweep) {
    ...
    for (size_t i = 0; i < all_values.size(); ++i) {
        ...
        // Reference: tanh with DAZ applied
        float daz_input = bf16_bits_to_float(bf16_daz_normalize(float_to_bf16_bits(input)));
        float expected_f64 = std::tanh(static_cast<double>(daz_input));
        float expected = bf16_bits_to_float(bf16_daz_normalize(float_to_bf16_bits(static_cast<float>(expected_f64))));
