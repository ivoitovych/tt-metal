...
PATCH_OUTPUT=$("${SCRIPT_DIR}/scripts/patch_activate_posix.sh" "$PYTHON_ENV_DIR" 2>&1)
PATCH_RC=$?
if [ $PATCH_RC -ne 0 ]; then
  echo "WARNING: patch_activate_posix.sh failed (rc=$PATCH_RC): $PATCH_OUTPUT" >&2
elif echo "$PATCH_OUTPUT" | grep -q "Skip"; then
