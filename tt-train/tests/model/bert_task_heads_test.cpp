...
TEST_F(BertHeadsTest, SequenceClassificationHeadForward) {
    ...
    auto pooled_input = core::from_vector(
        pooled_data, core::create_shape({batch_size, 1, 1, hidden_size}), autograd::ctx().get_device());
    ...
    auto output_shape = logits->get_value().get_logical_shape();
...
TEST_F(BertHeadsTest, TokenClassificationHeadForward) {
    ...
    auto seq_input = core::from_vector(
        seq_data, core::create_shape({batch_size, 1, seq_len, hidden_size}), autograd::ctx().get_device());
...
TEST_F(BertHeadsTest, QuestionAnsweringHeadForward) {
    ...
    auto output_shape = combined_logits->get_value().get_logical_shape();
    EXPECT_EQ(output_shape[0], batch_size);
    EXPECT_EQ(output_shape[1], 1);
    EXPECT_EQ(output_shape[2], seq_len);
    ...
    auto start_shape = qa_logits.start_logits->get_value().get_logical_shape();
...
TEST_F(BertTaskModelsTest, SequenceClassificationForward) {
    ...
    auto input_tensor =
        core::from_vector(input_ids, core::create_shape({batch_size, 1, 1, seq_len}), autograd::ctx().get_device());
...
TEST_F(BertTaskModelsTest, PreTrainingForward) {
    ...
    auto mlm_shape = output.mlm_logits->get_value().get_logical_shape();
    ...
    auto nsp_shape = output.nsp_logits->get_value().get_logical_shape();
...
TEST_F(BertLossesTest, SequenceClassificationLoss) {
    ...
    auto logits_tensor = core::from_vector(
        logits_data, core::create_shape({batch_size, 1, 1, num_labels}), autograd::ctx().get_device());
    auto logits = autograd::create_tensor(logits_tensor);

    // Create labels [B]
    std::vector<uint32_t> labels_data = {0, 1, 2, 0};
    auto labels_tensor = core::from_vector(labels_data, core::create_shape({batch_size}), autograd::ctx().get_device());
    ...
    auto loss_shape = loss->get_value().get_logical_shape();
    EXPECT_EQ(loss_shape.rank(), 0);
}
...
TEST_F(BertLossesTest, PreTrainingCombinedLoss) {
    ...
    auto mlm_tensor = core::from_vector(
        mlm_data, core::create_shape({batch_size, 1, seq_len, vocab_size}), autograd::ctx().get_device());
    ...
    auto nsp_tensor =
        core::from_vector(nsp_data, core::create_shape({batch_size, 1, 1, 2}), autograd::ctx().get_device());
    auto nsp_logits = autograd::create_tensor(nsp_tensor);

    // Create labels
    std::vector<int32_t> mlm_labels_data(batch_size * seq_len, -100);  // All masked
    mlm_labels_data[0] = 100;                                          // One unmasked token
    auto mlm_labels_tensor =
        core::from_vector(mlm_labels_data, core::create_shape({batch_size, seq_len}), autograd::ctx().get_device());
    auto mlm_labels = autograd::create_tensor(mlm_labels_tensor);

    std::vector<uint32_t> nsp_labels_data = {0, 1};
    auto nsp_labels_tensor =
        core::from_vector(nsp_labels_data, core::create_shape({batch_size}), autograd::ctx().get_device());
...
TEST_F(BertTaskModelsTest, EndToEndSequenceClassification) {
    ...
    EXPECT_TRUE(logits->get_grad() != nullptr);
}
