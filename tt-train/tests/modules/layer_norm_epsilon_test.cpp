...
// Test 10: Finite difference gradient validation for epsilon impact
TEST_F(LayerNormEpsilonTest, FiniteDifferenceGradientValidation) {
    const uint32_t features = 32;
    const float eps = 1e-5F;
    ...
    auto analytical_grad = tensor_to_vector(input->get_grad());

    // Compute numerical gradient using finite differences
    // For simplicity, test gradient w.r.t. first element
    autograd::ctx().reset_graph();

    // f(x + h)
    input_data[0] += finite_diff_h;
    auto input_plus_tt = core::from_vector(input_data, ttnn::Shape({1, 1, 1, features}), &autograd::ctx().get_device());
    ...
    // f(x - h)
    input_data[0] -= 2.0F * finite_diff_h;
    auto input_minus_tt =
    ...
    EXPECT_NEAR(analytical_grad[0], numerical_grad, 0.1F)
        << "Analytical gradient differs significantly from numerical gradient";
}
