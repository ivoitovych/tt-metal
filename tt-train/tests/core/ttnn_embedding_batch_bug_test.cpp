...
// TEST 4: Pass 4D tensor directly to ttnn::embedding (BERT scenario) - BUG REPRODUCTION
TEST_F(TtnnEmbeddingBatchBugTest, FourDTensorInput_ACTUAL_BUG_REPRODUCTION) {
    using namespace ttml;
    ...
    // Create input as 4D tensor [batch_size, 1, 1, seq_len] - EXACTLY like BERT does
    auto input_shape_4d = ttnn::Shape({batch_size, 1, 1, seq_len});
    std::vector<uint32_t> input_data(batch_size * seq_len);
    ...
    auto input_4d = core::from_vector<uint32_t, ttnn::DataType::UINT32>(
        input_data, input_shape_4d, device, ttnn::Layout::ROW_MAJOR);

    std::cout << "\n=================================================================\n";
    std::cout << "TEST 4: ACTUAL BUG REPRODUCTION - 4D tensor input\n";
    std::cout << "=================================================================\n";
    std::cout << "This test reproduces the EXACT scenario from BERT implementation:\n";
    std::cout << "  Input: 4D tensor [batch_size, 1, 1, seq_len]\n";
    std::cout << "  Passed DIRECTLY to ttnn::embedding() without reshape\n\n";
    std::cout << "Input configuration:\n";
    ...
    std::cout << "EXPECTED: Different token IDs should produce different embeddings\n";
    std::cout << "Calling ttnn::embedding() with 4D input (BERT scenario)...\n\n";

    // Call ttnn::embedding with 4D input - THIS IS THE BUG SCENARIO
    auto embeddings = ttnn::embedding(input_4d, weight_tensor, /* pad_token */ std::nullopt, ttnn::Layout::TILE);

    auto embeddings_shape = embeddings.logical_shape();
    std::cout << "Output shape: [" << embeddings_shape[0] << ", " << embeddings_shape[1] << ", " << embeddings_shape[2]
              << "]\n\n";
    ...
    // The output is [batch_size, seq_len, embedding_dim]
    for (uint32_t dim = 0; dim < embedding_dim; ++dim) {
        sample0_token0[dim] = embeddings_vector[0 * seq_len * embedding_dim + 0 * embedding_dim + dim];
        sample1_token0[dim] = embeddings_vector[1 * seq_len * embedding_dim + 0 * embedding_dim + dim];
    }
    ...
    if (max_diff < 0.001F) {
        std::cout << "❌ BUG CONFIRMED!\n";
        std::cout << "   Different token IDs (7 vs 99) produce IDENTICAL embeddings when\n";
        std::cout << "   ttnn::embedding() is called with 4D tensor [batch, 1, 1, seq]!\n";
        std::cout << "   This is the EXACT scenario from BERT implementation.\n";
        std::cout << "=================================================================\n\n";

        std::cout << "\nBUG REPRODUCTION EVIDENCE:\n";
        std::cout << "==========================\n";
        std::cout << "Function: ttnn::embedding()\n";
        std::cout << "Issue: Batch processing fails with 4D tensor input\n";
        std::cout << "Input shape: [" << batch_size << ", 1, 1, " << seq_len << "]\n";
        std::cout << "Sample 0 token IDs: all 7\n";
        std::cout << "Sample 1 token IDs: all 99\n";
        std::cout << "ACTUAL result: Identical embeddings (max diff=" << max_diff << ")\n";
        std::cout << "EXPECTED result: Different embeddings for different tokens\n";
        std::cout << "==========================\n\n";

        FAIL() << "ttnn::embedding batch processing bug CONFIRMED with 4D tensor input";
    } else {
        std::cout << "✓ CORRECT: Different token IDs produce different embeddings\n";
        std::cout << "   Bug may be fixed or test conditions don't trigger it\n";
        std::cout << "=================================================================\n\n";
    }
}
