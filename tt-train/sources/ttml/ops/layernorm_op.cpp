...
autograd::TensorPtr layernorm(
    ...
    // - BFLOAT16: 7-bit mantissa gives machine epsilon ~0.0078125. Values below ~1e-4F
    //   may truncate to zero, causing NaN in rsqrt(variance + eps). Clamp to 1e-4F minimum.
    ...
    const float safe_eps =
        (enable_hardware_clamp && dtype == tt::tt_metal::DataType::BFLOAT16) ? std::max(eps, 1e-4F) : eps;
